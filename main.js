...
$(function() {
  var $success = $('.completeness-indicator-success');
  if ($success.length >= SUCCESS_COUNT) {
    // 花火上げる

    var link = document.createElement("link");
    link.href = chrome.extension.getURL("fireworks.css");
    link.type = "text/css";
    link.rel = "stylesheet";
    document.getElementsByTagName("head")[0].appendChild(link);

    var gif = chrome.extension.getURL('particles.gif');

    $('body').append('<div id="fireworks-template"><div id="fw" class="firework"></div><div id="fp" class="fireworkParticle"><img src="' + gif + '" alt="" /></div> </div>');
    $('body').append('<div id="fireContainer"></div>');
    ...
    for(var i = 0; i < 15; i++) {
      createFirework(25,177,4,5,null,null,null,null,false,true);
    }
  }
});
